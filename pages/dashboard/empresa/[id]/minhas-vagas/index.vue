...
const items = ref([
  { id: 1, title: 'Analista de Marketing', status: 'Aberta para inscrição', ativa: true, dataCriacao: '12h30 12/09/2025' },
  { id: 2, title: 'Programador de Servidor', status: 'Encerrada', ativa: false, dataCriacao: '14h20 01/08/2025' },
  { id: 3, title: 'Designer de Experiência e Interface', status: 'Aberta para inscrição', ativa: false, dataCriacao: '09h45 30/07/2025' },
  { id: 4, title: 'Auxiliar Administrativo', status: 'Pausada', ativa: false, dataCriacao: '10h00 22/06/2025' },
  { id: 5, title: 'Coordenador de Projetos', status: 'Em processo seletivo', ativa: true, dataCriacao: '11h15 18/07/2025' },
  { id: 6, title: 'Técnico em Informática', status: 'Encerramento próximo', ativa: true, dataCriacao: '08h30 05/08/2025' },
  { id: 7, title: 'Especialista em Atendimento ao Cliente', status: 'Aberta para inscrição', ativa: true, dataCriacao: '15h00 25/07/2025' },
  { id: 8, title: 'Analista Financeiro', status: 'Cancelada', ativa: false, dataCriacao: '13h20 10/06/2025' },
])
...
const vagasFiltradas = computed(() => {
  return items.value.filter((vaga) => {
    const statusMatch = !selectedStatus.value || vaga.status === selectedStatus.value
    const ativaMatch =
...
<template>
  <v-row no-gutters>
    <v-col cols="12">
      <div class="d-flex flex-column">
        ...
        <span class="text-caption font-weight-bold">Se quiser, também crie uma nova.</span>
      </div>
    </v-col>
  </v-row>
  ...
  <v-row no-gutters>
    <v-col cols="12" :class="selectedStatus || selectedActive ? 'mt-4' : ''">
      <div class="d-flex">
        <v-btn text="Criar vaga" variant="flat" class="bg-gradient-primary" @click="$router.push('/dashboard/empresa/123/vagas/criar')" />
      </div>
    </v-col>
  </v-row>
  ...
  <v-row no-gutters>
      ...
      <v-card>
        <v-card-text class="pa-0">
          <v-list>
            ...
            <v-list-item v-if="vagasFiltradas.length === 0" class="px-4 text-grey">
              Nenhuma vaga encontrada com os filtros selecionados.
            </v-list-item>

            <v-list-item
              v-for="vaga in vagasPaginadas"
              :key="vaga.id"
            ...
            >
              ...
              >
                <div class="d-flex justify-space-between align-start">
                  <div>
                    <div class="text-subtitle-1 font-weight-medium">{{ vaga.title }}</div>
                    <div class="text-caption text-grey">
                      Status: {{ vaga.status }} |
                      Ativa: {{ vaga.ativa ? 'Sim' : 'Não' }}
                    </div>
                  </div>
                  <div class="text-caption text-grey-darken-1">{{ vaga.dataCriacao }}</div>
                </div>
              </v-card>
            </v-list-item>
          </v-list>
    ...
    <!-- Paginação -->
    <v-col v-if="currentPage < Math.ceil(vagasFiltradas.length / itemsPerPage)" cols="12" class="d-flex justify-center mt-4">
      <v-pagination
        v-model="currentPage"
        :length="Math.ceil(vagasFiltradas.length / itemsPerPage)"
        :total-visible="Math.ceil(vagasFiltradas.length / itemsPerPage)"
        color="primary"
        rounded
      />
    </v-col>
